...
def convert_to_480p(source):
    ...
    cmd = 'ffmpeg -y -i "{}" -s hd480 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(source, path_name)
...
def convert_to_720p(source):
    ...
    cmd = 'ffmpeg -y -i "{}" -s hd720 -c:v libx264 -crf 23 -c:a aac -strict -2 "{}"'.format(source, path_name)
    subprocess.run(cmd, shell=True)